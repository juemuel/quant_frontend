 ...
 */
   ...
   * 检测是否已经激活
   */
  checkCardActivated: function () {
    const cardStatus = localStorage.getItem('cardStatus')
    if (cardStatus === null || cardStatus === '' || cardStatus !== '2') {
      return false
    }
  ...
  certNoDesensitization (certNoStr: string) {
    ...
    if (certNoStr.length >= 18) {
      ...
      return this.desensitizedStrCommon(certNoStr, 4, 4)
    }
  },
  ...
  getRelationTypeColor: function (val: number) {
    switch (val) {
      case 0:
        return '#1989fa' // rgb(25,137,250)默认的primary颜色
      case 1:
        return '#71D2B9'
      case 2:
        return '#71D2B9'
