...
export default {
    ...
    // 创建分组
    createGroup: async function (params: { typeCode: string; name: string; ownerId: number }): Promise<any> {
        const url = process.env.VUE_APP_BASE_URL + '/common-api/group/create';
        return await http.post(url, params, {});
    },
    // 删除分组
    deleteGroup: async function (params: { groupId: number; ownerId: number }): Promise<any> {
        const url = process.env.VUE_APP_BASE_URL + '/common-api/group/delete';
        return await http.post(url, params, {});
    },
    // 更新分组
    updateGroup: async function (params: {
        groupId: number;
        typeCode: string;
        name: string;
        ownerId: number;
    ...
    // 获取分组列表（含分组项）
    getGroupList: async function (params: { userId: number; typeCode: string }): Promise<any> {
        const url = process.env.VUE_APP_BASE_URL + '/common-api/group/list';
        return await http.get(url, params, {});
    },

    // 添加分组项到某个分组
    addGroupItem: async function (params: { groupId: number; itemName: string; customData: any; ownerId: string }): Promise<any> {
        const url = process.env.VUE_APP_BASE_URL + '/common-api/group/item/add';
        return await http.post(url, params, {});
    },

    // 删除分组项
    deleteGroupItem: async function (params: { itemId: number; ownerId: number }): Promise<any> {
        const url = process.env.VUE_APP_BASE_URL + '/common-api/group/item/delete';
        return await http.post(url, params, {});
    },
    // 更新分组项
    updateGroupItem: async function (params: {
        groupItemId: number;
        groupId: number;
        notes?: string;
        itemName?: string;
