    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <div class="filter-container">
            <el-input v-model="data.searchKeyword" placeholder="输入股票代码或名称搜索" class="filter-item" clearable
              @clear="handleSearch" @keyup.enter="handleSearch" />
            <el-select v-model="data.industry" placeholder="行业分类" clearable @change="handleSearch" class="filter-item">
              <el-option v-for="item in data.industryOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
            ...
            <el-select v-model="data.priceChange" placeholder="涨跌幅" clearable @change="handleSearch"
              class="filter-item">
              <el-option v-for="item in data.priceChangeOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
      ...
      <div class="pagination-container">
        <el-pagination v-model:current-page="data.currentPage" v-model:page-size="data.pageSize"
          :page-sizes="[10, 20, 50, 100]" :total="data.total" layout="total, sizes, prev, pager, next"
