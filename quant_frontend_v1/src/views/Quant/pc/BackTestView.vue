  ...
  <div id="workingArea" class="page">
    ...
    <el-tabs v-model="data.activeProfitTab" @tab-click="handleProfitClick">
      <el-tab-pane label="策略交易统计" name="first">
        <el-table :data="data.tradeStatistics" stripe style="width: 100%">
          <el-table-column prop="totalTradeCount" label="总共交易次数" width="200" align="center"></el-table-column>
...
const simulate = async () => {
  try {
    ...
    quantApi.simulateTest(params).then((res) => {
      console.log('res', res)
      if (res.status === 200) {
        data.indexStartDate = indexStartDate;
        data.indexEndDate = indexEndDate;
        data.years = years
